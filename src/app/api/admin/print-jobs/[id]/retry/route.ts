...
export async function POST(
  ...
  if (job.status !== "failed") return NextResponse.json({ error: "Solo se pueden reintentar trabajos fallidos" }, { status: 409 });

  await PrintJob.updateOne(
    { _id: id, tenantId: session.tenantId },
    { $set: { status: "pending", attempts: 0, lastError: "", claimedAt: null, printedAt: null } }
  );
